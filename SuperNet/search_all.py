...
import tqdm
max_train_iters = 50
def valid_func(train_loader, xloader, network, criterion, valid_arch, logger):
...
#
    ...
    network.eval()
    valid_iter = iter(xloader)
    try:
      inputs, targets = next(valid_iter)
    except:
      valid_iter = iter(xloader)
      inputs, targets = next(valid_iter)
    _, logits = network(inputs.cuda(non_blocking=True), valid_arch) # ADDED cuda
...
def main(xargs):
  ...
  result_dir = xargs.output_dir + f'/{xargs.ckpt.split(".pth")[0]}_{xargs.edge_op}'
  ...
  for i in range(LEN):
    #valid_a_loss , valid_a_top1 , valid_a_top5 = valid_func(valid_loader, search_model, criterion, archs[i])
