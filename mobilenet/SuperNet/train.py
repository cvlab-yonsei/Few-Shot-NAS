...
def do_test(cfg, model, logger, checkpointer, testset, test_loader, CLASSES):
    evaluator = SemSegEvaluator(len(CLASSES), distributed=cfg.num_gpus > 1) 
    evaluator.reset()
    with torch.no_grad():
        model.eval()
...
def do_train(cfg, model, model_old=None):
    ...
    if cfg.MODEL.MIB_CLS_INIT and cfg.dataset != 'cityscapes_domain':
    ...
    model.train()
    ep = 1
    interval_eval = 1 #cfg.SOLVER.MAX_EPOCH
    interval_verbose = 1 + iters_per_epoch // 10
    ...
    for it, batch in zip(range(1, max_iter+1), train_loader):
        ...
        if cfg.STEP > 0:
            ...
            #gt[gt==0] = 255 # NOTE: This is to use CE (delete it when using UCE or training on Cityscapes)
...
def main(args):
    ...
    cfg.SAVE_NAME = f"{cfg.TAG}_{cfg.SEED}_"
    cfg.freeze()
    ...
    if cfg.dataset == 'cityscapes_domain':
        model = DeepLabV3([19], cfg.MODEL.SYNC_BN, freeze_type=cfg.MODEL.FREEZE_TYPE).to(torch.device("cuda"))
    else:
        model = DeepLabV3(num_classes, cfg.MODEL.SYNC_BN, freeze_type=cfg.MODEL.FREEZE_TYPE).to(torch.device("cuda"))
    model_old = None
    if cfg.STEP > 0:
        if cfg.dataset == 'cityscapes_domain':
            model_old = DeepLabV3([19], cfg.MODEL.SYNC_BN, freeze_type="all").to(torch.device("cuda"))
        else:
